...
void writeSPI(uint16_t word) 
{
	...
	{
    if(word & 0x8000) 
			SDATA(GPIO_PIN_SET);  //bit=1, Set High
		else 
			SDATA(GPIO_PIN_RESET);   //bit=0, Set Low
		dds_delay();
		DCLK(GPIO_PIN_RESET);             //Data is valid on falling edge
		dds_delay();
		DCLK(GPIO_PIN_RESET);  
